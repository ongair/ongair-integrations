...
class OngairTest
   ...
   #     expect_json({ success: true })
	  ...
	  it "should setup accounts" do
	  	params = { zendesk_url: "https://xyz.zendesk.com/api/v2", zendesk_access_token: "8OHrRib1QjB0lZN7GLreXv8fQNFp8Y7Ct629wi48",
	  	 ongair_token: "10qo6fb4al0sd2c58059f7dbd301f2b0", ongair_phone_number: "254772200061", zendesk_user: "xyz@gmail.com",
	  	  ongair_url: "http://app.ongair.im" }
	  	post '/accounts', params
	  	expect(subject).to receive(Zendesk.setup_account).with(params)
	  	# expect_json({:success => "true"})
	  end

	  it "should create tickets" do
	  	params = { account: "255686700088", subject: "hello", phone_number: "254722123890",
	  	 name: "John", notification_type: "MessageReceived", text: "help"}
	  	post '/tickets', params
	  	puts Account.count
	  	expect(subject).to receive(Zendesk.create_ticket).with(params, Account.first)
	  end
	end
end